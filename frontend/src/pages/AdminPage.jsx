    ...
    <div className="bg-[#f8f9fa] text-[#191c1d] font-sans min-h-screen">
            ...
            {NAV_ITEMS.map(item => {
              ...
              return (
                <li key={item.key}>
                  ...
                  >
                    <span className={`material-symbols-outlined text-[20px] flex-shrink-0 ${isActive ? 'text-[#004581]' : 'text-blue-300'
      ...
      <div className="md:ml-64 flex flex-col min-h-screen bg-[#f0f4f8]">
        ...
        <main className="flex-1 p-8 max-w-[1280px] w-full mx-auto">
          ...
          {/* DASHBOARD */}
          {activeNav === 'dashboard' && (() => {
            const doanVienActive = doanViens.filter(
              d => d.trangThaiSH === 'Đang sinh hoạt'
            ).length;

            const doanVienRutHoSo = doanViens.filter(
              d => d.trangThaiSH === 'Đã rút hồ sơ'
            ).length;

            const doanVienTotNghiep = doanViens.filter(
              d => d.trangThaiSH === 'Đã tốt nghiệp'
            ).length;
                  ...
                  <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {[
                        ...
                        { label: 'Rút hồ sơ', value: doanVienRutHoSo, color: '#f59e0b', bg: 'bg-amber-50', pct: doanViens.length > 0 ? Math.round(doanVienInact / doanViens.length * 100) : 0 },
                        { label: 'Tốt nghiệp', value: doanVienTotNghiep, color: '#f59e0b', bg: 'bg-amber-50', pct: doanViens.length > 0 ? Math.round(doanVienInact / doanViens.length * 100) : 0 },
